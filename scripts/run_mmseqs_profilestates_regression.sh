...
lap
"${MMSEQS}" search "${QUERYDB}" "${TARGETDB}" "${RESULTS}/aln_query_target" "${RESULTS}/tmp" -s 2 --e-profile 0.1 -e 0.1 --realign -a
lap
"${MMSEQS}" result2profile "${QUERYDB}" "${TARGETDB}" "${RESULTS}/aln_query_target" "${RESULTS}/query_profile"
lap
"${MMSEQS}" search "${RESULTS}/query_profile" "${RESULTS}/target_profile_states" "${RESULTS}/results_aln" "${RESULTS}/tmp" --max-seqs 4000 -e 100000.0 -s 2  --min-ungapped-score 0
lap
